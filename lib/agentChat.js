...
class AgentChat {
    ...
    processor(ev){
        ...
        if ((ev['@type'] === 'line') && (ev['source'] === 'visitor')) {
                ...
                var ob =this;
                ob.aiengine(ev.text,function(airesp){
                    if(airesp.result.action ==="input.unknown"||airesp.result.action=== "EscalationQuery"){
                        ob.availability((reqdata)=>{
                            var data = JSON.parse(reqdata);
                            var targetSkillId;
                            console.log(data)
                            console.log(JSON.stringify(data))
                            var skills= data.transfer.skill;
                            console.log("PRINTING AVAILABLITY STATUS",skills);
                            if(airesp.result.action=="input.unknown"){
                                for(let i=0;i<skills.length;i++){
                                    console.log("INSIDE FALLBACK")
                                    if(skills[i].name==="Live-Agent"){
                                        targetSkillId=skills[i].id
                                        console.log("SKILL",targetSkillId,skills[i].name)
                                        break;
                                    }
                                }

                            }else{
                                if(airesp.result.action=="EscalationQuery"){
                                    for(let i=0;i<skills.length;i++){
                                        console.log("INSIDE IDENTIFIED USECASE")
                                        if(skills[i].name==="Register-bot"){
                                            targetSkillId=skills[i].id
                                            console.log("SKILL",targetSkillId,skills[i].name)
                                            break;
                                        }
                        ...
                        console.log("Targer SkillId",targetSkillId);
                            ob.transfer(targetSkillId,(status)=>{
                            ...
                            if(airesp.result.fulfillment.speech===""){
                                ob.sendLine(airesp.result.fulfillment.messages[0].payload,"rich");
                            }else{
                                console.log(airesp.result.fulfillment.speech);
                                ob.sendLine(airesp.result.fulfillment.speech,"text");
                            }
                    }
                });
            }
