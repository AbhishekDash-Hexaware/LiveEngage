...
class AgentChat {
            ...
            if (events) {
                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    if ((ev['@type'] === 'state') && (ev.state === 'ended')) {
                        ...
                        var ob =this;
                        this.aiengine(ev.text,function(data){
                            ...
                            if(data.result.action ==="input.unknown"){
                                ...
                                ob.sendLine("tarsfering the control to agent");
                                }else{
                                    var API_mess = data.result.fulfillment.messages[0].speech;
                                // var API_mess="dummy text";
                                console.log(API_mess);
                                ob.sendLine(API_mess);
                            }
                        });
                    }
                }
    ...
    aiengine(usersays,callback){
        var options = { 
            ...
            qs: { v: '20150910' },
            headers: { 
                'postman-token': 'e9d37620-d5d8-ec0f-dbf3-5cee2ed2cfef',
                'cache-control': 'no-cache',
                authorization: 'Bearer b1456a3bb5a140e98d3e83569b80936c',
                'content-type': 'application/json'
            },
    ...
    sendLine(botsays) {
        ...
        console.log(`Sending line: ${botsays}`);
